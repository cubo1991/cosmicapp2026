...
export default function EditarCopa({ copaId, onSuccess }) {
  const { copa } = useCopa(copaId);
  ...
  useEffect(() => {
    if (copa) {
      setFormData({
        nombre: copa.nombre || "",
        descripcion: copa.descripcion || "",
        estado: copa.estado || "planificada",
        cantidadRondas: copa.reglas?.cantidadRondas || "",
      });
    }
  }, [copa]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMensaje({ tipo: "exito", texto: "¡Copa actualizada!" });
      setTimeout(() => {
  ...
  if (!copa) {
    return (
      <div className="p-6 text-center text-gray-600">Cargando copa...</div>
    );
  }

  return (
    <div className="w-full max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Editar Copa</h2>
